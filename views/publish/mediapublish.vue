<template>
    <div>
        ...
        <!-- '添加广告': '修改广告' -->
        <el-dialog :title="dialog_isAdd ? '添加广告': '修改广告'" :visible.sync="dialogFormVisible_publish" width="900px" @close="closeDialogFunc">
            <div class="m_aside">
            ...
            <div class="m_article">
                ...
                <div class="m_dialogMain">
                    <div class="box">
                        <ul class="m_listBox">
                            <li v-for="(item,$index) in tableData_media_dialog" :key="item.name" @mouseover="show_delete=$index" @mouseout="show_delete=null">
                                <dl>
                                    <dt>
                                        <img v-if="item.resourceType==2" class="media" :src="filePath + item.resourcePath" alt="">
                                        <video v-if="item.resourceType==1" class="media" :src="filePath + item.resourcePath"></video>
                                    </dt>
                                    <dd>
                                        资源类型：{{item.resourceType==1?'视频':'图片'}}
                                    </dd>
                                    <dd>
                                        资源名称：{{item.resourceName}}
                                    </dd>
                                    <dd>
                                        <span v-if="item.resourceType==2">分辨率：<em class="fbl_pic">{{item.imageWidth}}</em>X<em>{{item.imageHeight}}</em></span>
                                        <span v-if="item.resourceType==1">分辨率：<em class="fbl_pic">{{item.videoWidth}}</em>X<em>{{item.videoHeight}}</em></span>
                                        <span v-if="item.resourceType==1">视频时长：<em>{{item.duration}}</em></span>
                                    </dd>
                                    <dd v-if="show_delete==$index" class="delete" @click="deleteMediaFunc($index)">删除</dd>
                                </dl>
                            </li>
                        </ul>
                    </div>
                ...
                <el-dialog 
                    ...
                    <div class="m_dialogHead">
                        <el-row>
                            ...
                            <el-col :span="12">
                                <label>资源类型：</label>
                                <el-select clearable v-model="resourseType_CC" placeholder="请选择" size="small" @change="selectCC_Func">
                        ...
                        <el-row>
                             <el-button type="primary" round size="small" @click="redirectFunc('/resource')">添加广告</el-button>
                        </el-row>
                    </div>
                    <div class="m_dialogMain">
                        <div class="box">
        ...
        <el-dialog title="详情" :visible.sync="dialogFormVisible_detail" width="900px">
            ...
            <div class="m_article">
                ...
                <div class="m_dialogMain">
                    <div class="box">
                        <ul class="m_listBox">
                            <li v-for="item in tableData_media_dialog" :key="item.name">
                                <dl>
                                <dt>
                                    <img v-if="item.resourceType==2" class="media" :src="filePath + item.resourcePath" alt="">
                                    <video controls v-if="item.resourceType==1" class="media" :src="filePath + item.resourcePath"></video>
                                </dt>
                                <dd>
                                    资源类型：{{item.resourceType==1?'视频':'图片'}}
                                </dd>
...
export default {
    data(){
        return {
            ...
            tableData_mediapublish: [

            ],

            tableData_media_dialog: [
            ],
    ...
    methods:{
        deleteMediaFunc(index) {
            console.log(index);
            var delData=this.tableData_media_dialog.splice(index,1);
        ...
        redirectFunc(url){
            ...
            this.$store.state.user_module.switchTo_resource_dialogOpen_media=true;
        },
            ...
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                ...
                .then(function (response) {
                    ...
                    if (response.data.code==0) {
                        ...
                        _this.http_mediapublishTableFunc(data);
                    }else{
                        _this.$message({
                            type: 'info',
        ...
        queryProgramFunc(){
            ...
            this.http_mediapublishTableFunc(data);
        },
        ...
        http_mediapublishTableFunc(data){
            var _this=this;
                this.$get('/app/content-aggr/content/showResourcePublishRecordPage',data)
                .then(function (response) {
                    console.log(response);
                    if (response.data.code==0) {
                        _this.tableData_mediapublish=response.data.data.records;
        ...
        http_detailResourceFunc(datas){
            ...
            .then(function (response) {
                ...
                if (response.data.code==0) {
                    ...
                    debugger;
                   _this.tableData_media_dialog=response.data.data;
                }else{
                    _this.$message({
                        type: 'info',
                        message: response.data.msg
        ...
        //     debugger;
        //     var delData=this.tableData_media_dialog.splice(this.tableData_media_dialog.indexOf(data.row),1);
        //     this.return_deleteResourceIdsFunc(delData);
        ...
        http_resourceTableFunc(data){
            ...
            .then(function (response) {
                console.log(response);
                debugger
                var datas=response.data.data.records,
                    tableData_resource=[];
                for (let i = 0,len=datas.length; i < len; i++) {
                    var data=datas[i];
                    debugger;
                    if (data.resourceType==1) {
                        data.fenbianlv=data.videoWidth+'x'+data.videoHeight;
                    }else if (data.resourceType==2) {
                        data.fenbianlv=data.imageWidth+'x'+data.imageHeight;
                    }
                    tableData_resource[i]=data;
                }
        ...
        addSaveFunc_CC(){
            ...
            var allDatas=this.return_noRepeatDatasFunc(this.selectionData_CC_media,this.tableData_media_dialog);
            debugger;
            this.tableData_media_dialog=allDatas;
        ...
        publish_saveFunc(){
            ...
            var checkedKeys=this.$refs.ref_tree.getCheckedKeys(),
                resourceData=this.tableData_media_dialog,
                addResourceIds=[],
                addAreaIds=[],
                deleteAreaIds=[];
            // var is_danyuan_id=['0c3e1160e46c4be0a0c5a51a31bde859','057773b62ba14b16bccecea04cb7cc64']
            // var checkedKeys=checkedKeys.filter((item, index) =>{
            //     // debugger;
            //     return is_danyuan_id.indexOf(item)>=0;
            // })
            ...
            if (this.tableData_media_dialog.length<1) {
                this.$message({
                    type: 'info',
                    message: '资源列表不可为空'
                });
    ...
    getters:{
        // filePath(){
        //     return this.$store.getters.getFilePath;
        // }
    },
    created() {
        var data={
