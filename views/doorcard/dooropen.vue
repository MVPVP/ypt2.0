        ...
        <div class="f-contain">
            <div class="box">
                ...
                <div class="contain">
                    <el-row class="btnBox">
                        <el-col :span="24">
                            <el-input v-model="cardSn" style="width: 300px;"></el-input>
                            <el-button type="primary" round size="small" @click="btnReadIdCardFunc">读卡</el-button>
                        </el-col>
                    </el-row>
                    <el-row v-if="cardStatus==1 || cardStatus==2">
                        <el-col :span="5">
                            <label for="">卡片状态：</label><span>{{cardStatus==1 ? '新建' : '激活'}}</span>
                        </el-col>
                        <el-col :span="8">
                            <label for="">卡ID：</label><span>{{cardSn}}</span>
                        </el-col>
                        <el-col :span="5">
                            <label for="">卡片类型：</label><span>{{cardType | cardType_filt}}</span>
                        </el-col>
                        <el-col :span="6">
                            <label for="">备注：</label>
            ...
            <div class="box">
                ...
                <div class="contain">
                    <div class="noCardBox" v-if="ifBindLive == 1">
                        <el-row class="btnBox">
                            <el-col :span="24">
                    ...
                    <div class="m-cardBox"  v-if="ifBindLive == 2">
                        <div class="header">
                            <span class="name">
...
<script>
import {card_interfaceFunc,readIdCardFunc,arrayToAttrArray,formateDate,tree_addAttr,tree_arrayToAttrArray_childrens , tree_addAttr_b} from '@/utils/common.js'
...
export default {
    data(){
        return{
            ...
            cardType: 1,//1:IC; 2:CPU; 3:NFC; 4:ID; 5:身份证; 6:居住证
            ifBindLive: 1,//1为没绑定用户没授权；2为绑定用户或者授权
            cardStatus: 1,//1为未开卡，2为已开卡
            // cardId: '2c3351d64aba477f9f15b0c3491b3cde',
            // 新建：f9f4184898bb40e386903ccdc5ca6999
            // 激活：410ce6f791634fad9effda938e13bc3b
            cardSn: 'f9f4184898bb40e386903ccdc5ca6999',
            cardId: null,
    ...
    methods:{
        ...
        backFunc(){
            this.$router.push({path: '/dooropen'});
        },
        card_bindPersonFunc(){
            // debugger
        ...
        btnReadIdCardFunc(){
            ...
            this.http_queryCardIsExist(data);
        },
        http_queryCardInfoFunc(datas){//没用到的？0905
            var _this=this;
            this.$get('/app/hpc-aggr/hpc/queryCardInfo',datas)
            .then(function (response) {
                console.log(response);
                debugger;
                if (response.data.code==0) {
        ...
        http_queryCardIsExist(datas){
            var _this=this;
            this.$get('/app/hpc-aggr/hpc/queryCardIsExist',datas)
            .then(function (response) {
                console.log(response);
                debugger;
                if (response.data.code==0) {
                    _this.if_load_treeAgain= false;
                    var isExist= response.data.data.isExist;//1为未开卡，2为已开卡
                    _this.cardStatus= isExist;
                    _this.cardId= response.data.data.cardId;
                    if (isExist==1) {
                        _this.tableData_houseList= [];
                        _this.ifBindLive= 1;
                        ...
                        _this.if_load_treeAgain= true;
                        sessionStorage.setItem('disabledNodes_session','[]');
                    }else if (isExist==2) {
                        var ddd={
                            managementId: '6b0c4ccab56711e88a4a000c29a401f2',
                        ...
                        _this.http_getCardBindPersonInfo(ddd);
                    }
                }else{
                    _this.$message({
                        type: 'info',
        ...
        doorCard_saveFunc(){
            ...
            var datas={
                ...
                cardId: this.cardStatus==1 ? null : this.cardId,
                cardSn: this.cardStatus==1 ? this.cardSn : null,
                cardType: this.cardStatus==1 ? this.cardType : null,
        ...
        http_doorCardSaveFunc(datas){
            ...
            .then(function (response) {
                ...
                if (response.data.code==0) {
                    ...
                    _this.http_queryCardIsExist(data);
                }else{
                    _this.$message({
                        type: 'info',
                        message: response.data.msg
    ...
    created() {
        ...
        this.http_queryCardInfoFunc(ddd);
    },
    watch:{
    ...
    filters:{//cardType: 1,//1:IC; 2:CPU; 3:NFC; 4:ID; 5:身份证; 6:居住证
        cardType_filt(val){
            if (val==1) {
                return 'IC'
            }
